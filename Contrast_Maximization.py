...
from event_process import warp
...
def contra_max(x, y, p, t, t_ref, rangeX, rangeY):
    res = minimize(variance, np.array([0, 0]), args=(x, y, p, t, t_ref, rangeX, rangeY), method='nelder-mead', options={'xatol': 1e-8, 'disp': False})
    optimize.fmin_cg(variance, np.array([0, 0]), args=(x, y, p, t, t_ref, rangeX, rangeY))
    return res.x